...
import { Lock, CheckCircle, Users, ArrowRight, ChartBar } from '@phosphor-icons/react';
    ...
    <div className="pt-24 pb-16 min-h-screen">
      <div className="max-w-4xl mx-auto px-6">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-ink mb-4">{t('rewards.title')}</h1>
          <p className="text-xl text-subtext">{t('rewards.subtitle')}</p>
        </div>
        ...
        <div className="bg-white rounded-3xl p-8 shadow-xl border border-blue-50 mb-12">
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              <h3 className="text-2xl font-bold text-ink">{t('rewards.inviteRewards')}</h3>
              <p className="text-subtext">{t('invite.subtitle')}</p>
            </div>
          </div>
          <Link to="/invite">
            <Button variant="candy" className="w-full md:w-auto">
        ...
        <div className="bg-white rounded-3xl p-8 shadow-xl border border-orange-50 mb-12">
          <h3 className="text-2xl font-bold text-ink mb-8">{t('rewards.poolsSplits')}</h3>
          
          <SplitBar 
            label="Revive (10 BWT) → Treasure Pool" 
            splits={[
          ...
          <SplitBar 
            label="Endless Ticket (20 BWT) → Leaderboard Pool" 
            splits={[
          ...
          <SplitBar 
            label="Item Packs" 
            splits={[
              { width: '50%', color: 'bg-orange-500', text: '50% Burn' },
              { width: '10%', color: 'bg-blue-500', text: '10% Ref' },
              { width: '40%', color: 'bg-green-500', text: '40% Dev' },
            ]} 
          />
        </div>
        ...
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-3xl p-8 shadow-xl text-white">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div>
              <h3 className="text-2xl font-bold mb-2">{t('rewards.transparency')}</h3>
              <p className="text-gray-400 mb-6">{t('transparency.subtitle')}</p>
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/10 rounded-xl p-3">
                  <p className="text-xs text-gray-400 mb-1">{t('transparency.cards.burn')}</p>
                  <p className="font-bold">1.25M</p>
                </div>
                <div className="bg-white/10 rounded-xl p-3">
                  <p className="text-xs text-gray-400 mb-1">{t('transparency.cards.pool')}</p>
                  <p className="font-bold">580K</p>
                </div>
              </div>
            </div>
            <Link to="/transparency">
              <Button variant="ghost" className="bg-white/10 hover:bg-white/20 text-white border-white/20">
                {t('transparency.cta')} <ChartBar weight="bold" />
              </Button>
            </Link>
          </div>
        </div>
